...
const InternationalSection = () => {
  ...
  useEffect(() => {
    // Fetch news from the API for अन्तर्राष्ट्रिय category
    const fetchInternationalNews = async () => {
      try {
        ...
        if (data.status && data.data) {
          // Filter news for "अन्तर्राष्ट्रिय" category and take first 3
          const filteredInternational = data.data
            .filter(item => item.category === 'अन्तर्राष्ट्रिय')
            .slice(0, 3)
            .map(item => ({
              id: item.id,
              title: item.heading,
          ...
          setInternationalNews(filteredInternational);
        }
      } catch (err) {
  ...
  if (internationalNews.length === 0) {
    return (
      <section>
        <SectionHeader title="अन्तर्राष्ट्रिय" href="/category/antarrashtriya" />
        <div className="text-center py-4 text-gray-500">
          कुनै अन्तर्राष्ट्रिय समाचार छैन
        </div>
      </section>
    );
  }
    ...
    <section>
      ...
      <div>
        {internationalNews.map((item, i) => (
          ...
          >
            <ListCard item={item} seed={`int_${i}`} />
          </div>
        ))}
      </div>
    </section>
